...
exports.getPlatformSlug = getPlatformSlug;
exports.getPlatformCollection = getPlatformCollection;
exports.getTechTitle = getTechTitle;
exports.getQuickstartMetdata = getQuickstartMetdata;
...
function getQuestion(platformType) {
...
function getPlatformCollection(quickstart, platformType) {
  if (!platformType) {
    return [];
  }
...
function getTechTitle(quickstart, appType, techName) {
  var collection = getPlatformCollection(quickstart, appType);

  var result = _lodash2['default'].find(collection, { name: techName });
...
function getQuickstartMetdata(quickstart, appType, tech1, tech2) {
  var meta = {};
...
function loadArticle(payload) {
  var url = (payload.baseUrl || '') + '/' + getPlatformSlug(payload.appType) + '/' + (payload.currentTech + '?' + payload.appType + '=' + payload.tech1 + '&e=1');
  if (payload.tech2) {
